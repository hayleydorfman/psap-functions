...
#model options:
## steal = steal/steal+a; steal/1+a; steal*a - compare these three models; then look at dist and run spearman corrs
...
aggFunc <- function(a, betaT, participant, data = alltrialsdat) { 
  ...
  valSteal <- singsub$stealTrial*a # value of steal is reward amt x a param - instead do we want to do stealtrial/(stealtrial+a)
  valBank <- singsub$earnTrial    
  loglike2 <- ifelse(singsub$choiceType=="steal",
                    logSumExp(log(1/(1+exp(-betaT*(valSteal-valBank))))) , 
                    logSumExp(log(1-1/(1+exp(-betaT*(valSteal-valBank))))))
  loglike <- sum(-loglike2)     
...
for (participant in u.ppl[1:n]) {
  ...
  fr <- function(x) {
    ...
    min(result,10^10)
   # trial <- nrow(alltrialsdat)
    #return(tempError)
  }
  
  output <- optim( par=c(0.5,0.5), fn=fr, gr=NULL, method="L-BFGS-B", lower=c(0.001,0.01), upper=c(5,1))
  
  cat("participant:", participant, "best agg:", output$par[2], "best am:", output$par[1], output$value, "\n")
  
  data <- list(participant, output$par[2], output$par[1], output$value)
  ...
  names(df)<- c("participant", "best agg", "best tempr", "LL")
  
 
}

names(mainDat) <- c("participant", "best agg", "best tempr","LL")
write.csv(mainDat, file = paste(c("hyper_model3_agg_comb_model_summary_01", ".csv"), collapse = ""))
...
# a - FIX
plot_agg_param <- ggplot(alltrialsdat, aes(x = stealTrial, y = output$par[2])) +
  geom_point(aes(color = choiceType)) +
  facet_wrap(~ participant) +
  theme_bw()
ggsave(plot_disagg_param, file = "plot_aggParam.pdf", height = 9, width = 11)
plot_agg_param
