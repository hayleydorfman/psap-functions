...
#-----------Survey Correlations with Agg Param------------------#
#Read in surveys
combdf <- read.csv("/Users/hayley/Dropbox (Harvard SNP Lab)/PSAP ITC/Code/Final Code/PROV/surveys.csv")

# Read in agg param
agg<- read.csv("/Users/hayley/Dropbox (Harvard SNP Lab)/PSAP ITC/PSAP_PRO_V/Analyses/PROV_model_summary_harmparam.csv")
dim(agg)
aggDat <- merge(agg,combdf, by = "participant")

combdf1 <- aggDat[,c(3,10:41)]

cor1 <- cor.test (combdf1$best.agg, combdf1$sub_physical_aggression)
cor1 #SIG
plot(combdf1$best.agg, combdf1$sub_physical_aggression)

corr3 <- corr.test(combdf1, combdf1, method = "spearman", use = "pairwise", adjust = "none")
write.csv(corr3$r, file=paste("/Users/hayley/Dropbox (Harvard SNP Lab)/PSAP ITC/PSAP_PRO_V/Analyses/", format(Sys.time(), "%a_%b_%d_%H:%M:%S_%Y"), "_allCorrs_PROV_uncorr_all_spear_r.csv", sep = ""), row.names = T)
write.csv(corr3$p, file=paste("/Users/hayley/Dropbox (Harvard SNP Lab)/PSAP ITC/PSAP_PRO_V/Analyses/", format(Sys.time(), "%a_%b_%d_%H:%M:%S_%Y"), "_allCorrs_PROV_uncorr_all_spear_p.csv", sep = ""), row.names = T)

plot(combdf1$total_buss_perry,combdf1$best.agg)
plot(combdf1$stab_subscore_physical_aggression,combdf1$best.agg)
plot(combdf1$ppi_sf_mach,combdf1$best.agg)
plot(combdf1$stab_subscore_social_aggression,combdf1$best.agg)
plot(combdf1$sub_perseverance,combdf1$best.agg)
...
choices$Choice = factor(choices$Choice,levels=c("bank", "steal"),ordered=TRUE)
...
png("test.png", width = 12, height = 8, units = 'in', res = 300)
...
corrDat <- read.csv("combSummary.csv")
