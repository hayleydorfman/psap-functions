...
aggFunc <- function(a, betaT, participant, data = alltrialsdat) { 
  ...
  valSteal <- singsub$stealTrial/singsub$stealTrial+(a*singsub$harmTrial) # value of steal is reward amt/reward amt + a param * harm magnitude
...
for (participant in u.ppl[1:200]) {
...
write.csv(mainDat, file = paste(c("PROV_model_summary_harmparam_01", ".csv"), collapse = ""))
